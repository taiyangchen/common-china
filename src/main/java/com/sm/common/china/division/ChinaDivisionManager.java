package com.sm.common.china.division;
...
import com.sm.common.china.convert.GBKMap;
 ...
 */
public class ChinaDivisionManager implements DivisionManager {
  ...
  /** 部定义编号对应的对象 */
  private final Map<Integer, ChinaDivision> idMap = new HashMap<>();
  /** 国标缩写对应的对象 */
  private final Map<String, List<Division>> abbNameMap = new HashMap<>();
  /** 名字对应的对象 */
  private final Map<String, List<Division>> nameMap = new HashMap<>();
  /** 邮编对应的对象 */
  private final Map<String, List<Division>> zipMap = new HashMap<>();
  ...
  private ChinaDivision create(String[] strings) throws Exception {
    ...
    if (strings[3] != null && StringUtils.isNotBlank(strings[4])) {
  ...
  private void transformData(List<String> lines) {
    for (String line : lines) {
      ...
      String[] strings = StringUtils.split(line, ",", LINE_ITEMS);
  ...
  public List<Division> getObscureDivisionByName(String name) {
    ...
    for (Map.Entry<String, List<Division>> entry : nameMap.entrySet()) {
      ...
      if (StringUtils.contains(divisionName, name)) {
  ...
  public boolean isCityDivision(int id) {
    ...
    if ((chinaDivision != null) && (chinaDivision.getParentDivision() != null)
        && (chinaDivision.getParentDivision().getParentDivision() != null) && (chinaDivision.getParentDivision()
  ...
  public boolean isRegionDivision(int id) {
    ...
    if ((chinaDivision != null) && (chinaDivision.getParentDivision() != null)
        ...
        && (chinaDivision.getParentDivision().getParentDivision().getParentDivision()
